...
passport.use(new FacebookStrategy({
    ...
    profileFields: ['id', 'displayName', 'emails']  // Fields to fetch from Facebook
    ...
    try {
        ...
        db.query('SELECT * FROM user2 WHERE facebook_id = ?', [id], async (err, results) => {
            if (err) return done(err);

            if (results.length > 0) {
                if (!results[0].facebook_id) {
                    db.query('UPDATE user2 SET facebook_id = ? WHERE email = ?', [id, email]);
                }
